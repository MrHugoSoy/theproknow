-- feed_page: búsqueda de texto (p_query) y modo 'guardados'.
--  · p_query   : filtra por título o cuerpo (ILIKE, con %, _ y \ escapados). También sirve #etiquetas.
--  · guardados : posts guardados por el usuario, ordenados por fecha de guardado (más reciente primero).
-- Los envoltorios feed_nuevos/siguiendo/tendencias/para_ti siguen llamando con 7 argumentos
-- (p_query queda en su valor por defecto).

drop function if exists public.feed_page(text, int, double precision, uuid, text, text, timestamptz);
...
declare
  ...
  v_q   text := case when p_query is null or btrim(p_query) = '' then null
                     else '%' || replace(replace(replace(btrim(p_query), '\', '\\'), '%', '\%'), '_', '\_') || '%' end;
begin
  if p_mode not in ('nuevos', 'siguiendo', 'tendencias', 'para_ti', 'guardados') then
  ...
  from (
    select
      ...
      (case
         ...
         when p_mode = 'guardados'
           then floor(extract(epoch from sv.created_at) * 1000)::double precision
         else
      ...
      (sv.created_at is not null)                                                             as viewer_saved,
    ...
    left join lateral (
      select s2.created_at from public.saves s2
      where v_uid is not null and s2.user_id = v_uid and s2.post_id = p.id
    ) sv on true
    where p.created_at <= v_now
      ...
      and (p_mode <> 'guardados' or sv.created_at is not null)
...
revoke execute on function public.feed_page(text, int, double precision, uuid, text, text, timestamptz, text) from public;
grant  execute on function public.feed_page(text, int, double precision, uuid, text, text, timestamptz, text) to anon, authenticated;
